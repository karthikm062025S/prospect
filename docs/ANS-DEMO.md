...
Each agent (Profile, Match, Roadmap, Orchestrator) holds an ANS passport: a name like `ans://v1.0.1.roadmap.prospect.courses` plus a signed TL record proving the registration authority verified and sealed it. Before any agent run writes a row, `startAgentRun` (`lib/student-profile.ts:78`) calls `assertVerifiedAgent` (`lib/ans-verify.ts`) — a badge that is not `ACTIVE` on the TL is refused before the write. The RA/TL are the ANS reference stack, run by us today and hostable by anyone; the app trusts the signed log, never a flag in its own database.

## Why it is not on GoDaddy's list

- **Log-backed verification.** The badge comes from a signed Transparency Log record, not a self-issued flag in our own database.
...
```
Published 2026-09-20 03:40 ET and verified through Cloudflare 1.1.1.1: all four `_ans-badge.<agent>` TXT rows, the four A rows, the four SVCB rows and the four TLSA rows are live, and each `https://<agent>.prospect.courses/.well-known/mcp/server-card.json` returns 200 (TLSA/SVCB show via DNS-over-HTTPS, since Windows nslookup cannot query those types). From `ans/dns-records.json`: SVCB at `roadmap.prospect.courses`, TXT at `_ans-badge.roadmap.prospect.courses`, TLSA at `_443._tcp.roadmap.prospect.courses`.
