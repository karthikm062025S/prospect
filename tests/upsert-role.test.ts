...
const seeded = (title: string, season: string, family: string | null) =>
  fakeSupabase({
    companies: [{ id: "co-1", name: "Acme" }],
    roles: [{ id: "r-1", company_id: "co-1", title, posted_at: REFIND.posted_at, lifecycle: "open", season, family, location: null }],
  });

test("update leaves a stored season alone when the title alone says unspecified", async () => {
  const supabase = seeded("Engineering Intern", "summer_2027", "ai_ml");
  const result = await upsertRole(supabase, { ...REFIND, title: "Engineering Intern" });
...
test("update still self-heals an unspecified season and a null family from the title", async () => {
  const supabase = seeded("Software Engineer Intern Summer 2027", "unspecified", null);
  const result = await upsertRole(supabase, { ...REFIND, title: "Software Engineer Intern Summer 2027" });
  assert.equal(result.action, "update");
  const role = result.role as { season: string; family: string };
  assert.equal(role.season, "summer_2027");
...
// testable without a live Supabase client — mirrors how resolveRoleUpsert /
...
test("insert stamps last_seen_at and canonical_key, computed server-side", async () => {
  const supabase = fakeSupabase({ companies: [], roles: [] });
...
test("re-find re-stamps last_seen_at; a stale re-find (>7d) increments repost_count and refreshes posted_at on the SAME row; a fresh re-find (<7d) leaves both alone", async () => {
    ...
    roles: [
      {
        ...
        repost_count: 0,
        canonical_key: "co:co-1|t:software engineer intern|l:-",
      },
    ],
  };
  ...
  assert.equal(staleRole.id, "r-1", "must resolve to the SAME row via canonical_key");
  ...
  assert.equal(tables.roles.length, 1, "the repost must not insert a new row");
...
test("a stale re-find whose new posted_at is already held by a DIFFERENT row of the same company+title skips the posted_at refresh, but still increments repost_count and re-stamps last_seen_at", async () => {
  ...
  const tables = {
    ...
    roles: [
      ...
      },
      {
        ...
        repost_count: 0,
        canonical_key: "co:co-1|t:software engineer intern|l:different-location", // never matched via canonical_key
      },
    ],
  };
  const supabase = fakeSupabase(tables);
  ...
  assert.equal(result.action, "update");
  const role = result.role as Row;
  assert.equal(role.id, "r-1", "must still resolve to r-1 via canonical_key");
  ...
  assert.equal(tables.roles.length, 2, "no row should be inserted or removed by the guard");
});
      ...
      roles: [
        {
          ...
          location: null,
          last_seen_at: null,
          updated_at: "2026-09-01T00:00:00.000Z", // 15 days old
          canonical_key: "co:co-1|t:data analyst intern|l:-",
        },
      ],
    });
  const staleResult = await upsertRole(
      ...
      roles: [
        {
          ...
          location: null,
          last_seen_at: null,
          updated_at: "2026-09-15T00:00:00.000Z", // 1 day old
          canonical_key: "co:co-1|t:data analyst intern|l:-",
        },
      ],
    });
...
test("a payload whose link carries the same Workday id as a stored row but a different title resolves to that row (update), inserting nothing", async () => {
  ...
  assert.equal(result.action, "update");
  assert.equal((result.role as Row).id, "r-1");
  assert.equal(tables.roles.length, 1, "a different title on the same ATS id must never insert a duplicate");
});
...
test("when two stored rows share a canonical_key, the OLDEST (by created_at) is the one updated", async () => {
  ...
  assert.equal(result.action, "update");
  assert.equal((result.role as Row).id, "r-old", "the oldest matching row (by created_at) must be the one updated");
  assert.equal(tables.roles.length, 2, "no new row should be inserted");
});
...
test("a tombstone whose canonical_key matches (same Workday id, different title) blocks the insert", async () => {
  ...
  const tables = {
    companies: [{ id: "co-1", name: "Acme" }],
    roles: [],
    tombstones: [{ id: "t-1", company_id: "co-1", title: "Software Engineer Intern", posted_at: "2026-07-01", link }],
  };
  const supabase = fakeSupabase(tables);
  const result = await upsertRole(supabase, {
    company: "Acme",
  ...
  assert.equal(tables.roles.length, 0, "a canonical-key-matched tombstone must never let the posting re-insert");
});
...
test("a stored wd:R01171049 at company A does not match a payload carrying the same Workday id for company B — inserts a new row under B, leaves A untouched", async () => {
  ...
  assert.notEqual(newRole.company_id, "co-a", "the new row must belong to company B, not A");
  assert.equal(newRole.canonical_key, "wd:R01171049");

  const rowA = tables.roles.find((r) => r.id === "r-a") as Row;
  ...
  assert.equal(tables.roles.length, 2, "two rows must now exist: A's original and B's new insert");
});
