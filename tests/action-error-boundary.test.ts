...
test("a reader that returns a PostgREST error is mapped the same way", async () => {
  const failing = {
    from: () => ({
      select: () => ({
        eq: () => ({
          maybeSingle: async () => ({ data: null, error: { message: 'column roles.jd_error does not exist' } }),
        }),
      }),
    }),
  } as unknown as SupabaseClient;
  ...
  assert.equal(res.error, "column roles.jd_error does not exist");
...
test("getRoleJdAction routes every lower-layer error through publicError", () => {
  ...
  assert.match(src, /publicError\(error\.message\)/);
